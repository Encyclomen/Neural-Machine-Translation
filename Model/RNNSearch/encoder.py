...
class Encoder(nn.Module):
    ...
    def init_hidden(self, batch_size):
        """
        :param batch_size(int): the number of sentences contained in a batch
        :return h0(Variable): the initialized bidirectional encoder hidden states, not yet cancatenated
        """
    ...
    def forward(self, input, mask):
        ...
        length = mask.sum(1).tolist()  # 从mask中获得batch中每个句子的长度，mask矩阵由0，1元素构成
        packed_input_emb = torch.nn.utils.rnn.pack_padded_sequence(input_emb, length, batch_first=True)
        enc_hiddens, _ = self.bi_gru(packed_input_emb, hidden)
