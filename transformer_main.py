...
from Optim.transformer_optim_wrapper import Transformer_Optim_Wrapper
...
def parse_args():
    ...
    saved_model = 'Transformer_max_bleu_model'
    checkpoint = 'Transformer_checkpoint_model'
...
# all options-----------------------------------------------------------------------------------------------------------
    ...
    # dataset, corpus and batch parameters
    ...
    parser.add_argument('--train_batch_size', type=int, default=2, help='the batch size used in training')
    parser.add_argument('--src_max_len', type=int, default=100, help='the max length of valid sentence in source language corpus')
    parser.add_argument('--trg_max_len', type=int, default=100, help='the max length of valid sentence in target language corpus')
    # vocab parameters
    ...
    parser.add_argument('--interval', type=int, default=4, help='the batch interval for parameter updating')
    ...
    parser.add_argument('--d_q', type=int, default=64, help='')
    ...
    parser.add_argument('--enc_N', type=int, default=6, help='')
    parser.add_argument('--dec_N', type=int, default=6, help='')
    parser.add_argument('--d_inner_hid', type=int, default=1024, help='')
    ...
    # Misc.
    parser.add_argument('--mode', type=str, default='train', help='the working mode of the model')
...
def my_callback(result):
    ...
    if bleu > max_bleu:
        ...
        batch_idx = result[1]
        cur_epoch = result[2]
        save_max_bleu_model(model, opt.saved_model_dir, batch_idx, cur_epoch, max_bleu*100, info='Transformer_max_bleu_model')
...
def train(model, src_vocab, trg_vocab, optim_wrapper, train_iter, vldt_iter, loss_function):
    ...
    for epoch in range(opt.epoch, opt.nepoch):  # TODO
        ...
        for batch_idx, batch in enumerate(train_iter, start=1):
            ...
            # --------------------------------------- TODO
            f_trg = torch.cat((f_trg, torch.LongTensor([[dec_pad] for _ in range(int(f_trg.size(0)))])), 1)
            ...
            if batch_idx % opt.interval == 0:
                ...
                if total_loss < min_loss:
                    ...
                    min_loss = total_loss
                    save_min_loss_model(model, opt.checkpoint_dir, batch_idx/opt.interval, cur_epoch, min_loss, info='Transformer_min_loss_model')
                else:
        ...
        save_checkpoint_model(model, opt.checkpoint_dir, cur_epoch, info='Transformer_checkpoint_model')
...
def evaluate(opt, model, src_vocab, trg_vocab, corpus_iter, batch_idx, cur_epoch):
    try:
        ...
        print('subprocess %d batch_idx %d, time: ' % (os.getpid(), batch_idx), time1)
...
def translate(model, src_vocab, trg_vocab, corpus_iter, translation_output=None):
    ...
    for idx, batch in enumerate(corpus_iter, start=1):
        ...
        src_mask = get_batch_mask(src, src_vocab, PAD)
        with torch.no_grad():
            sentence = model.greedysearch(src, src_mask, cuda=opt.cuda)
            sentence = batch_idx2str([sentence], trg_vocab)
            hyp_list.append(sentence)
            '''
            sentences_output, scores_output = model.beamsearch(src, src_mask, opt.beam_size, normalize=True)
            best_sentence, best_score = sentences_output[0], scores_output[0]
            best_sentence = batch_idx2str([best_sentence], trg_vocab)
...
if __name__ == "__main__":
    ...
    print("*******************Start Transformer*******************", os.getpid())
    ...
    if opt.mode == 'train':
        ...
        if opt.if_load_checkpoint:
            model = load_model(Transformer,
                               opt.d_model, opt.d_q, opt.d_k, opt.d_v, opt.h, opt.enc_N, opt.dec_N, opt.d_inner_hid,
                               opt.dropout, enc_ntok, dec_ntok,
        ...
        optimizer = load_optimizer(model, torch.optim.Adam,
                                   betas=(0.9, 0.98), eps=1e-9, weight_decay=opt.weight_decay_l2)
        optim_wrapper = load_optim_wrapper(optimizer, Transformer_Optim_Wrapper,
                                   d_model=opt.d_model, n_warmup_step=opt.n_warmup_step)
        ...
        loss_function = load_loss_function(torch.nn.CrossEntropyLoss, reduce=True, ignore_index=dec_pad)
        train(model, src_vocab, trg_vocab, optim_wrapper, train_dataloader, vldt_dataloader, loss_function)  # start training
    else:
        # load the assigned model
        model = load_model(Transformer,
                           opt.d_model, opt.d_q, opt.d_k, opt.d_v, opt.h, opt.enc_N, opt.dec_N, opt.d_inner_hid,
