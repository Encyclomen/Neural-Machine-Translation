...
class Decoder(nn.Module):
    ...
    def forward(self, prev_trg_emb, dec_hidden, enc_mask, enc_hiddens):
        """
        :param trg_emb: the word embedding of previous target word y(i-1)
        ...
        :return: dec_output: the batch of decoder's outputs at current timestamp i for all sentences
                  hidden: the batch of decoder hidden states s(i) at current timestamp i for all sentences
        """
        dec_hidden_proposal = self.gru1(prev_trg_emb, dec_hidden)  # 将s(i-1)先与y(i-1)融合，得到s'(i-1)作为decoder在i-1时刻的hidden state proposal
